...
<x-app-layout>
    ...
    @php
        ...
        $dpMode    = old('digital_pass_mode', 'off');
        $dpMethods = old('digital_pass_methods', 'both');
    @endphp
    ...
    >
        ...
        <form id="create-event-form" action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data" @submit.prevent="validateAndSubmit()">
            ...
            <section x-show="step === 1" x-cloak class="space-y-6">
                <div class="form-card">
                    ...
                    <div class="mt-4 flex flex-wrap gap-6 text-sm">
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" class="form-radio" value="free" x-model="pricing" name="pricing">
                            <span>Free event</span>
                        </label>
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" class="form-radio" value="paid" x-model="pricing" name="pricing">
                    ...
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4" x-show="pricing==='paid'">
                        <div>
                            ...
                            <select name="ticket_currency" x-model="currency" class="form-select">
                                <template x-for="c in currencies" :key="c">
                                    <option :value="c" x-text="c"></option>
                                </template>
                            </select>
                            <p class="form-help">We’ll map currency to payout country automatically.</p>
                        </div>
                    </div>
                </div>

                {{-- Ticket Types --}}
                <div class="form-card" x-show="pricing==='paid'">
                    <div class="flex items-center justify-between">
                        <h3 class="form-section-title">Ticket types</h3>
                    </div>
                    ...
                    <div id="cat-rows" class="mt-4 space-y-3">
                        ...
                        @forelse($cats as $i => $c)
                            <div class="cat-row border border-slate-200 bg-slate-50 p-3">
                                <div class="grid grid-cols-1 sm:grid-cols-12 sm:gap-3">
                                    ...
                                    <div class="sm:col-span-1 mt-3 sm:mt-0 flex sm:items-end sm:justify-end">
                                        <button type="button" class="text-rose-600 text-sm remove-cat">Remove</button>
                                    </div>
                                </div>
                            </div>
                        @empty
                    ...
                    <template id="cat-tpl">
                        <div class="cat-row border border-slate-200 bg-slate-50 p-3">
                            <div class="grid grid-cols-1 sm:grid-cols-12 sm:gap-3">
                                <div class="sm:col-span-5">
                                    <label class="form-label">Ticket name</label>
                                    <input name="__IDX__[name]" class="form-input" placeholder="e.g., VIP" :required="pricing==='paid'" :disabled="pricing!=='paid'">
                                </div>
                                <div class="sm:col-span-3 mt-3 sm:mt-0">
                                    <label class="form-label">Price</label>
                                    <input type="number" step="0.01" min="0" name="__IDX__[price]" class="form-input" placeholder="0.00" :required="pricing==='paid'" :disabled="pricing!=='paid'">
                                </div>
                                <div class="sm:col-span-3 mt-3 sm:mt-0">
                                    <label class="form-label">Capacity (optional)</label>
                                    <input type="number" min="0" name="__IDX__[capacity]" class="form-input" placeholder="e.g., 100" :disabled="pricing!=='paid'">
                                </div>
                                <div class="sm:col-span-1 mt-3 sm:mt-0 flex sm:items-end sm:justify-end">
                                    <button type="button" class="text-rose-600 text-sm remove-cat">Remove</button>
                                </div>
                    ...
                    <button type="button" id="add-cat" class="mt-3 form-primary-btn">
                        + Add another ticket type
                    </button>
                </div>

                <div class="form-card" x-show="pricing==='paid'">
                    <h3 class="form-section-title">Platform fee</h3>
                    <p class="text-sm text-slate-600 mt-1">
                        ...
                        This option CANNOT be modified after creating the event.
                    </p>

                    <div class="mt-4 space-y-3 text-sm">
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="radio" class="form-radio mt-1"
                                name="fee_mode" value="absorb"
                                @checked(old('fee_mode', 'absorb') === 'absorb')>
                            <div>
                                <div class="font-medium text-slate-900">Organiser absorbs fee</div>
                        ...
                        <label class="flex items-start gap-3 cursor-pointer">
                            ...
                            <div>
                                <div class="font-medium text-slate-900">Pass fee to attendees</div>
                                <div class="text-slate-600">
                ...
                <div class="form-card" x-show="pricing==='paid'">
                    <h3 class="form-section-title">Payout destination</h3>
                    <template x-if="eligibleMethods.length">
                        <div class="mt-3 grid grid-cols-1 gap-3">
                            <template x-for="m in eligibleMethods" :key="m.id">
                                <label class="cursor-pointer border p-3 flex items-start gap-3"
                    ...
                    <div class="mt-3 border border-amber-300 bg-amber-50 p-4 text-amber-900 text-sm"
            ...
            <section x-show="step === 2" x-cloak class="space-y-6">
                <div class="form-card">
                    ...
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <p class="form-help">Type and press enter to add. You can add multiple.</p>
                        </div>

                        <div class="sm:col-span-2">
                            <label class="form-label">Location</label>
                            <input id="location-input" type="text" name="location" value="{{ old('location') }}" class="form-input" placeholder="Venue, address or place name" autocomplete="off">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="border border-slate-300 rounded-none overflow-hidden">
                ...
                <div class="form-card">
                    ...
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                        <label class="flex items-start gap-2 border rounded-none px-3 py-2 cursor-pointer
            ...
            <section x-show="step === 3" x-cloak class="space-y-6">
                <div class="form-card">
                    ...
                    <div id="sessions-wrapper" class="mt-4 space-y-4">
                        <h5 class="text-slate-700 mt-1 text-sm">You can add up to 6 upcoming sessions below.</h5>
                        <div class="session-item border border-slate-200 bg-slate-50 p-4">
                    ...
                    <button type="button" id="add-session" class="mt-3 form-primary-btn">
                        + Add another session
                    </button>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <button type="button" @click="goStep(2)" class="form-secondary-btn">Back</button>
                    <button type="submit" :disabled="pricing==='paid' && !chosenMethodId" class="form-primary-btn disabled:opacity-50">
                        Create Event
                    </button>
                </div>
            </section>
    ...
    <script>
            ...
            Alpine.data('createEvent', (cfg) => ({
                ...
                goStep(n){
                    if (n > 1 && this.pricing==='paid' && !this.chosenMethodId) {
                        alert('Select a payout destination for ' + this.country);
                        return;
                    }
                    this.step = n;
                ...
                validateAndSubmit() {
                    if (this.pricing === 'paid') {
                        ...
                        if (!valid) {
                            alert('Add at least one ticket type with a positive price.');
                            return;
                        }
                        if (!this.chosenMethodId) {
                            alert('Please choose a payout destination.');
                            return;
                        }
                    }
        ...
        (function () {
            ...
            addBtn.addEventListener('click', () => {
                const html = `
                    <div class="session-item border border-slate-200 bg-white p-4">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="text-sm font-medium text-slate-800">Session</h4>
