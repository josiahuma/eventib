                ...
                <div class="mb-4 flex items-center gap-2 text-sm">
                    ...
                    @if($pos > 0)
                        <a class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200"
                           href="{{ route('tickets.show', ['event'=>$event,'registration'=>$registration,'ticket'=>$siblings[$pos-1]]) }}">
                            ‹ Prev
                        </a>
                    @endif
                    @if($pos < $count-1)
                    ...
                    <select class="ml-auto border rounded px-2 py-1"
                        ...
                        @foreach($siblings as $i => $s)
                            <option value="{{ route('tickets.show', ['event'=>$event,'registration'=>$registration,'ticket'=>$s]) }}"
